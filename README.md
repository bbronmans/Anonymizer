...
Links staat de naam van de grafische kaart, die je https://www.nvidia.com/download/index.aspx moet invoeren om de bijbehorende drivers te downloaden. Sluit het Nvidia Control Panel voor je de installatie van de nieuwe drivers start.
...
In deze "weights" map, moet je de volgende twee bestanden zetten:
    ...
    Gezichten: 
    https://drive.google.com/file/d/1CwChAYxJo3mON6rcvXsl82FMSKj82vxF
...
10. De 1-na laatste stap: voer het volgende commando in; **Let op: Met deze stap wordt een paar GB (~7.5 GB) aan software gedownload en geïnstalleerd, dit zal een tijd duren.**   
```
docker build -t anon .
   ```
...
*Let op: Windows heeft soms de neiging om dit bestand te verbergen. Zie je het bestand niet en wil je het zichtbaar maken? In Windows Explorer staat bovenaan onder Beeld de optie om verborgen bestanden te tonen.*
...
7. Er verschijnt in de command prompt wat tekst waaronder een kopie van de gebruikte anonymizer-instellingen, en na een paar seconden zal de output folder langzaam vollopen met geanonimiseerde afbeeldingen. Het proces is voltooid wanneer de volgende tekst getoond wordt: 
```
